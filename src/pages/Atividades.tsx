...
export default function Atividades() {
  ...
  const {
    ...
    calcA, duracaoSomada, escAutoMap, lang
  } = useApp();
  ...
  const duracaoTotal = (() => {
    ...
    return (mesFimMax !== null && mesMin !== null) ? (mesFimMax - mesMin + 1) : 0;
  })();

  const groups: Array<['M' | 'L', string, string]> = [
    ...
    <div style={S.pg}>
      <Card>
        <div style={{ padding: "12px 16px", display: "flex", alignItems: "baseline", gap: 8 }}>
          <span style={{ fontSize: 10, color: C.txt3, fontWeight: 600, letterSpacing: 1 }}>
            {t("activities.durationBase")}
          </span>
          <span style={{ fontSize: 22, fontWeight: 800, color: duracaoTotal > 0 ? C.gold : C.txt3, lineHeight: 1 }}>
            {duracaoTotal > 0 ? duracaoTotal : "—"}
          </span>
          {duracaoTotal > 0 && <span style={{ fontSize: 10, fontWeight: 600, color: C.txt2 }}>{t("gantt.months").toUpperCase()}</span>}
          {!duracaoSomada && mesMin != null && mesFimMax != null && (
            <span style={{ fontSize: 10, color: C.txt3, marginLeft: 8 }}>
              {t("activities.monthPeriod", { min: mesMin, max: mesFimMax })}
            </span>
          )}
        </div>
      </Card>

      {groups.map(([grp, col, label]) => (
        <Card key={grp}>
          <Hdr2 col={col} ch={label} />
          <table style={S.tbl}>
            <thead><tr>
              <TH ch={t("activities.cols.activity")} w={290} />
              <TH ch={t("activities.cols.und")} right w={60} />
              <TH ch={t("activities.cols.volume")} right w={130} accent />
              <TH ch={t("activities.cols.kpi")} right w={150} />
              <TH ch={t("activities.cols.startMonth")} right w={100} />
              <TH ch={t("activities.cols.comment")} w={200} />
            </tr></thead>
            <tbody>
              {ATIVS.filter(a => a.grp === grp).map((a, i) => (
                <tr key={a.id} style={S.trOff(i)}>
                  ...
                  <td style={{ padding: "5px 8px", textAlign: "right" }}>
                    <Tag text={a.und[currentLang]} col={col} />
                  </td>
                  ...
                  <td style={{ padding: "4px 8px" }}>
                    <LocalTextInp
                      ...
                      placeholder={t("activities.placeholders.comment")}
                      w="100%"
                    />
                  </td>
                </tr>

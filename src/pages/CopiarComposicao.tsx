  ...
  const copiar = async (opt: any) => {
    ...
    try {
      ...
      for (const row of valid) {
        updateComp(gIdx, row.atividade_id, (cur: any) => ({
          ...cur,
          moRows:  row.mo_rows  ?? [],
          eqRows:  row.eq_rows  ?? [],
  ...
  if (!grupoId) {
    return (
      <div style={{ ...S.app, display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh" }}>
        <div style={{ fontSize: 11, color: C.txt3, letterSpacing: 2 }}>{t("common.loading").toUpperCase()}</div>
      </div>
    );
  }
      ...
      <div style={{ width: "100%", maxWidth: 520, padding: "0 20px" }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 9, color: C.txt3, letterSpacing: 2, fontWeight: 700, marginBottom: 12 }}>
            {t("copier.selectSession")}
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 18 }}>
            {copyOptions.map((opt: any) => {
              ...
              return (
                ...
                >
                  ...
                  <span style={{
                    ...
                    {ativo ? t("copier.copying") : t("copier.useThis")}
                  </span>
                </button>
              );
            })}

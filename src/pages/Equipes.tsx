...
interface SenhaInputProps {
  grupoId: string;
  onSave: (id: string, plaintext: string) => void;
}

function GrupoField({ value, onSave, placeholder = "" }: GrupoFieldProps) {
...
function SenhaInput({ grupoId, onSave }: SenhaInputProps) {
  ...
  return (
    <input
      type="password"
      value={v}
      placeholder={t("groups.placeholders.pass")}
      onChange={e => setV(e.target.value)}
...
export default function Equipes() {
  ...
  const saveSenha = (id: string, plaintext: string) => uGrupo(id, "senha", plaintext);

  return (
    <div style={S.pg}>
      <Card>
        <Hdr2 ch={t("groups.titleShort")} right={
          <button style={S.btnS} onClick={addGrupo}>+ {t("common.add").toUpperCase()}</button>
        } />
        <table style={S.tbl}>
          <thead><tr>
            <TH ch="#" w={30} />
            <TH ch={t("groups.cols.name").toUpperCase()} />
            <TH ch={t("groups.cols.leader").toUpperCase()} />
            <TH ch={t("groups.cols.passShort").toUpperCase()} w={160} />
            <TH ch="" w={40} />
          </tr></thead>
          <tbody>
            {grupos.map((g, i) => (
              <tr key={g.id} style={S.trOff(i)}>
                ...
                <td style={{ padding: "4px 8px" }}>
                  <GrupoField 
                    value={g.resp} 
                    placeholder={t("groups.placeholders.leader")} 
                    onSave={v => uGrupo(g.id, "resp", v)} 
                  />
                </td>
                <td style={{ padding: "4px 8px" }}>

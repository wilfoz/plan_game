...
export default function Composicao() {
  ...
  const moOpts = MO_CAT.filter(r => !moUsados.has(r.id)).map(r => ({ 
    id: r.id, 
    label: r.cargo[currentLang] 
  }));

  const eqOpts = EQ_CAT.map(r => ({ 
    id: r.id, 
  ...
  const availReqOpts = reqsAtiv
    ...
    .map(r => {
      const displayDesc = currentLang === "es" ? (REQ_TRANSLATIONS[r.desc] || r.desc) : r.desc;
  ...
  return (
    <div style={S.pg}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 16, fontWeight: 700, color: C.goldL }}>{fmt(totalGeral, currentLang)}</div>
        <div style={{ marginLeft: "auto", fontSize: 10, color: C.txt2 }}>
          {t("composition.kpisDefined", { count: ATIVS.filter(a => gc(gIdx, a.id).kpi > 0).length, total: ATIVS.length })}
        </div>
      </div>
      ...
      <div style={S.g2}>
        ...
        <div>
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontSize: 11, fontWeight: 700, color: colGrp }}>{aObj.desc[currentLang]}</div>
          ...
          {/* MÃO DE OBRA */}
          <Card>
            <Hdr2 col={C.blueL} ch={`${t("composition.moTitle")} — ${aObj.desc[currentLang].slice(0, 28)}`} />
            <table style={S.tbl}>
              <thead><tr>
            ...
            <div style={{ padding: "8px 12px", borderTop: `1px solid ${C.border}`, display: "flex", gap: 8, alignItems: "center" }}>
              ...
              {moOpts.length > 0 && (
                <span style={{ fontSize: 10, color: C.txt3, whiteSpace: "nowrap" }}>
                  {t("composition.moSelectCount", { count: MO_CAT.length - moOpts.length, total: MO_CAT.length })}
                </span>
              )}
            </div>
          </Card>
          ...
          {/* EQUIPAMENTOS */}
          <Card>
            <Hdr2 col={C.yellow} ch={`${t("composition.eqTitle")} — ${aObj.desc[currentLang].slice(0, 28)}`} />
            <table style={S.tbl}>
              <thead><tr>
            ...
            <div style={{ padding: 14 }}>
              <table style={{ ...S.tbl, marginBottom: 14 }}>
                <tbody>
                  <tr style={{ borderBottom: `2px solid ${C.border2}`, background: C.surf2 }}>
                    <td style={{ padding: "8px 9px", fontSize: 12, fontWeight: 700, color: C.goldL }}>
                      {t("composition.summaryCols.costActivity")}
                    </td>
                    <td style={{ padding: "8px 9px", textAlign: "right", fontSize: 14, fontWeight: 700, color: C.goldL }}>
                      {fmt(calc.total, currentLang)}
                    </td>
                  </tr>
                  {[
                  ...
                  {calc.fatorMobilizacao > 1 && (
                    <tr style={{ borderBottom: `1px solid ${C.border}`, background: C.yellow + "0A" }}>
                      ...
                      <td style={{ padding: "5px 9px", textAlign: "right", fontSize: 11, fontWeight: 700, color: C.yellow }}>
                        +{fmt(calc.custoMobilizacao, currentLang)}
                      </td>
                    </tr>
                  )}
                  <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                    <td style={{ padding: "6px 9px", fontSize: 11, color: C.txt2 }}>{t("composition.summaryCols.duration")}</td>
              ...
              {role !== "G" && ef.temSubAlocacao && (
                <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: 10, marginBottom: 10 }}>
                  <div style={{ fontSize: 9, color: C.redL, letterSpacing: 3, marginBottom: 6 }}>{t("composition.alertsTitle")}</div>
              ...
              <div style={{ padding: "10px 12px", borderRadius: 5, background: C.surf3, border: `1px solid ${C.border2}` }}>
                ...
                <div style={{ fontSize: 17, fontWeight: 700, color: C.goldL }}>{fmt(totalGeral, currentLang)}</div>
              </div>
            </div>
          </Card>
        </div>

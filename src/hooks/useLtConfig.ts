...
import { LtConfig } from "../types";

export function rowToLt(row: any): LtConfig | null {
  if (!row) return null;
  return {
    nome:         row.nome          ?? "",
...
function ltToRow(lt: LtConfig, sessionId: string) {
  return {
...
export function useLtConfig(sessionId: string | null) {
  const qc = useQueryClient();
  const debounceRef = useRef<any>(null);

  const query = useQuery<LtConfig | null>({
  ...
  const upsertDebounced = (lt: LtConfig) => {

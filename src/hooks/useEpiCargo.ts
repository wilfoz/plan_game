...
export type EpiCargoMap = Record<string, Record<string, boolean>>;

// Returns { [mo_cat_id]: { [epi_cat_id]: true } }
function rowsToMap(rows: any[]): EpiCargoMap {
  const map: EpiCargoMap = {};
  for (const r of rows ?? []) {
...
export function useEpiCargo(sessionId: string | null) {
  const qc = useQueryClient();
  const key = ["epi_cargo", sessionId];

  const query = useQuery<EpiCargoMap>({
    queryKey: key,
    enabled: !!sessionId,
    queryFn: async () => {
      if (!sessionId) return {};
  ...
  const toggle = useMutation({
    mutationFn: async ({ moCatId, epiCatId }: { moCatId: string; epiCatId: string }) => {
      if (!sessionId) return;
